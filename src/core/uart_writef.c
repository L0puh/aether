...
volatile USART_t* external_uart = NULL;

static void out_char(char c) {
   if (external_uart == NULL) {
      uart_putchar(c);
   } else {
      uart_putchar_ex((USART_t*) external_uart, c);
   }
}
...
void uart_writef_va(const char *str, va_list args) {
   va_list args_copy;
   ...
   for (int i = 0; str[i] != '\0'; i++) {
      ...
      switch (conv) {
         case 's':
            out_str(va_arg(args_copy, char*));
            break;

         case 'c':
            out_char((char)va_arg(args_copy, int));
            break;

         case 'd':
         case 'i': {
                      long val = spec.is_long ? va_arg(args_copy, long) : (long)va_arg(args_copy, int);
                      int neg = val < 0;
         ...
         case 'u': {
                      unsigned long val = spec.is_long ? va_arg(args_copy, unsigned long)
                         : (unsigned long)va_arg(args_copy, unsigned int);
         ...
         case 'p':
                   emit_ptr(va_arg(args_copy, void*));
                   break;
...
void uart_writef_ex(USART_t* uart, const char* str, ...)
{
   external_uart = uart;
   if (external_uart == NULL) {
      return;
